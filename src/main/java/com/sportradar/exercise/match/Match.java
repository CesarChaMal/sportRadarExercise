...
public abstract class Match implements MatchInterface, Comparable<Match> {
    ...
    protected Match(Builder builder) {
        this.id = setIdAndIncrementIdGenerator(builder);
    ...
    private Long setIdAndIncrementIdGenerator(Builder builder) {
        final Long id;
        lock.lock();
        try {
            if (builder.id != null) {
                id = builder.id;
                idGenerator.updateAndGet(current -> Math.max(current, builder.id + 1));
//                logger.info(("CustomId: " + idGenerator.get());
            } else {
                id = idGenerator.getAndIncrement();
//                logger.info(("AutoGeneratedId = " + idGenerator.get());
            }
        } finally {
            lock.unlock();
        }
...
//        updateScore(currentHomeScore, newAwayScore);
    ...
    public abstract static class Builder<T extends Builder<T>> {
        ...
        protected Function<Match, EventManager> eventManagerFactory;
...
//            ensureSingleStrategy();
        ...
        public T eventManagerFactory(Function<Match, EventManager> eventManagerFactory) {
            this.eventManagerFactory = Objects.requireNonNull(eventManagerFactory, "Event manager factory must not be null");
            return self();
        }
